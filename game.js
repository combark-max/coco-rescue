...
const distanceBarEl = document.querySelector("#distanceBar");
const healthBarEl = document.querySelector("#healthBar");
const weaponBarMeterEl = document.querySelector("#weaponBarMeter");
...
function updateMeters() {
  ...
  const distanceRatio = Math.max(0, Math.min(1, -state.player.y / Math.abs(stage.goalY)));
  const healthRatio = state.player.maxHealth ? state.player.health / state.player.maxHealth : 0;
  const weapon = activeWeapon();
  const ammoRatio = weapon.maxAmmo ? ammoForWeapon(weapon) / weapon.maxAmmo : 0;
  const shieldRatio = state.shieldActive ? state.shieldHealth / 55 : state.shields / 3;
  setMeter(distanceBarEl, distanceRatio);
  setMeter(healthBarEl, healthRatio);
  setMeter(weaponBarMeterEl, ammoRatio);
  setMeter(shieldBarEl, shieldRatio);
}
...
function updatePlayUiVisibility() {
  ...
  shieldToggle.classList.toggle("hidden", !showPlayUi);
  weaponToggle.textContent = activeWeapon().icon;
  weaponToggle.setAttribute("aria-pressed", String(weaponPickerOpen));
  shieldCountEl.textContent = `🛡 ${state.shields}`;
  shieldToggle.disabled = !state.running || state.shields <= 0 || state.shieldActive;
  shieldToggle.setAttribute("aria-pressed", String(state.shieldActive));
  weaponBar.classList.toggle("hidden", !showPlayUi || !weaponPickerOpen);
  updateMeters();
}

function activateShield() {
  if (!state.running || state.paused || state.shieldActive || state.shields <= 0) return;
  state.shields -= 1;
  state.shieldActive = true;
  state.shieldHealth = 55;
  state.shieldTime = 6;
  playTone({ frequency: 820, duration: 0.14, type: "triangle", volume: 0.14 });
  setTimeout(() => playTone({ frequency: 1040, duration: 0.16, type: "triangle", volume: 0.1 }), 90);
  updatePlayUiVisibility();
}

function absorbDamage(damage, x = state.player.x, y = state.player.y) {
  if (state.shieldActive && state.shieldHealth > 0) {
    const blocked = Math.min(damage, state.shieldHealth);
    state.shieldHealth -= blocked;
    damage -= blocked;
    addSpark(x, y, "#89c7ff");
    playTone({ frequency: 720, duration: 0.06, type: "sine", volume: 0.08, slideTo: 420 });
    if (state.shieldHealth <= 0) {
      state.shieldActive = false;
      state.shieldTime = 0;
      playNoise(0.16, 0.16, 900);
      updatePlayUiVisibility();
    }
  }
...
function startStage(stageIndex) {
  ...
  state.boss = null;
  if (stage.id === "sea") {
    state.boss = { type: "sea", x: 0, y: stage.goalY + 230, health: 45, maxHealth: 45, speed: 78, active: false, shotCooldown: 0.9, bombCooldown: 2.4 };
  } else if (stage.id === "space") {
    state.boss = { type: "space", x: 0, y: stage.goalY + 300, health: 85, maxHealth: 85, speed: 96, active: false, shotCooldown: 0.62, bombCooldown: 1.8 };
  }
  state.pickups = [
  ...
  while (enemySpawnY > state.player.y - stage.enemyLookAhead) {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      state.enemies.push({
        ...
        kind,
        shotCooldown: 0.8 + Math.random() * 1.4,
      });
    }
...
function updateBoss(dt) {
  ...
  if (Math.hypot(dx, dy) < (boss.type === "space" ? 58 : 48)) {
    absorbDamage((boss.type === "space" ? 58 : 42) * dt, boss.x, boss.y);
    addSpark(boss.x, boss.y, boss.type === "space" ? "#ff6fbd" : "#4aa3df");
  }

  boss.shotCooldown -= dt;
  boss.bombCooldown -= dt;
  if (boss.shotCooldown <= 0) {
    fireBossShot(boss);
    boss.shotCooldown = boss.type === "space" ? 0.72 : 1.0;
  }
  if (boss.bombCooldown <= 0) {
    throwBossBomb(boss);
    boss.bombCooldown = boss.type === "space" ? 2.0 : 2.7;
  }
}
...
function hurtPlayerForCocoHit(coco, damage) {
  if (coco.hit) return;
  coco.hit = true;
  absorbDamage(damage, coco.x, coco.y);
...
function fireEnemyShot(enemy) {
  ...
  const dir = normalize(dx, dy);
  const speed = enemy.kind === "alien" ? 310 : enemy.kind === "diver" ? 260 : 240;
  state.enemyShots.push({
    ...
    vy: dir.y * speed,
    life: 2.2,
    size: enemy.kind === "alien" ? 6 : 5,
    damage: enemy.kind === "alien" ? 13 : 9,
    color: enemy.kind === "alien" ? "#8cffd2" : "#ff7a7a",
    kind: "bullet",
  });
  playTone({ frequency: enemy.kind === "alien" ? 520 : 300, duration: 0.05, type: "square", volume: 0.055, slideTo: 170 });
}
...
function fireBossShot(boss) {
  ...
  const spread = boss.type === "space" ? [-0.22, 0, 0.22] : [-0.12, 0.12];
  for (const offset of spread) {
    const vx = dir.x * Math.cos(offset) - dir.y * Math.sin(offset);
    const vy = dir.x * Math.sin(offset) + dir.y * Math.cos(offset);
    state.enemyShots.push({
      x: boss.x + right.x * 12,
      y: boss.y + right.y * 12,
      vx: vx * (boss.type === "space" ? 340 : 300),
      vy: vy * (boss.type === "space" ? 340 : 300),
      life: 2.4,
      size: boss.type === "space" ? 7 : 6,
      damage: boss.type === "space" ? 15 : 12,
      color: boss.type === "space" ? "#ff6fbd" : "#7bdcff",
      kind: "bullet",
    });
  }
  playTone({ frequency: boss.type === "space" ? 180 : 230, duration: 0.1, type: "sawtooth", volume: 0.085, slideTo: 90 });
}
...
function throwBossBomb(boss) {
  ...
  state.enemyShots.push({
    ...
    vx: dir.x * 185,
    vy: dir.y * 185,
    life: boss.type === "space" ? 1.25 : 1.45,
    size: boss.type === "space" ? 13 : 11,
    damage: boss.type === "space" ? 24 : 18,
    color: boss.type === "space" ? "#b16cff" : "#ff9f43",
    kind: "bomb",
    blastRadius: boss.type === "space" ? 92 : 78,
  });
  playTone({ frequency: 120, duration: 0.18, type: "triangle", volume: 0.11, slideTo: 60 });
}

function explodeEnemyBomb(shot) {
  if (shot.exploded) return;
  shot.exploded = true;
  const radius = shot.blastRadius || 76;
  if (Math.hypot(state.player.x - shot.x, state.player.y - shot.y) < radius) {
    absorbDamage(shot.damage, shot.x, shot.y);
    playEventSound("hurt");
  }
  for (let i = 0; i < 3; i += 1) addSpark(shot.x, shot.y, shot.color || "#ff9f43");
  playNoise(0.24, 0.2, 260);
}
...
function update(dt) {
  ...
  const backingOnly = state.input.y > 0.55 && Math.abs(state.input.x) < 0.22;
  if (backingOnly) {
    absorbDamage(18 * dt, state.player.x, state.player.y);
    if (Math.random() < 0.08) addSpark(state.player.x, state.player.y, "#ff8f70");
  }
  ...
  for (const shot of state.enemyShots) {
    ...
    shot.y += shot.vy * dt;
    shot.life -= dt;
    if (shot.kind === "bomb" && shot.life <= 0) explodeEnemyBomb(shot);
  }
  ...
  for (const enemy of state.enemies) {
    ...
    if (Math.hypot(dx, dy) < 30) {
      absorbDamage(22 * dt, enemy.x, enemy.y);
      if (Math.random() < 0.04) playEventSound("hurt");
      addSpark(enemy.x, enemy.y, "#d64747");
    }

    enemy.shotCooldown -= dt;
    const canShoot = Math.abs(enemy.y - state.player.y) < 620 && Math.abs(enemy.x - state.player.x) < width * 0.35;
    if (canShoot && enemy.shotCooldown <= 0) {
      fireEnemyShot(enemy);
      enemy.shotCooldown = (enemy.kind === "alien" ? 0.9 : enemy.kind === "diver" ? 1.15 : 1.35) + Math.random() * 0.75;
    }
  }
  ...
  for (const shot of state.enemyShots) {
    ...
    const hitRadius = shot.kind === "bomb" ? 26 : 18;
    if (Math.hypot(shot.x - state.player.x, shot.y - state.player.y) < hitRadius + shot.size) {
      if (shot.kind === "bomb") {
        explodeEnemyBomb(shot);
      } else {
        shot.exploded = true;
        absorbDamage(shot.damage, shot.x, shot.y);
        playEventSound("hurt");
        addSpark(state.player.x, state.player.y, shot.color || "#ff7a7a");
      }
    }
  }
  state.enemyShots = state.enemyShots.filter((shot) => shot.life > 0 && !shot.exploded);
...
function drawPlayer() {
  ...
  if (state.shieldActive) {
    ...
    ctx.fillStyle = "rgba(137, 199, 255, 0.16)";
    ctx.beginPath();
    ctx.arc(0, -4, 48, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = "rgba(137, 199, 255, 0.88)";
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.arc(0, -4, 48, 0, Math.PI * 2);
    ctx.stroke();
...
function draw() {
  ...
  for (const shot of state.enemyShots) {
    const screen = worldToScreen(shot.x, shot.y);
    if (shot.kind === "bomb") {
      ctx.fillStyle = shot.color || "#ff9f43";
      ctx.beginPath();
      ctx.arc(screen.x, screen.y, shot.size || 11, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = "#fff08a";
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.arc(screen.x, screen.y, (shot.size || 11) + 5, 0, Math.PI * 2 * Math.max(0.15, shot.life % 1));
      ctx.stroke();
    } else {
      ctx.fillStyle = shot.color || "#ff7a7a";
      ctx.beginPath();
      ctx.arc(screen.x, screen.y, shot.size || 5, 0, Math.PI * 2);
      ctx.fill();
    }
  }
...
shieldToggle.addEventListener("click", activateShield);
